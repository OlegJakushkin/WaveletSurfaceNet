...
"                       #   DIVERSE teachers, not all of them). ~10-25 s/mesh on a T4 (runs fully on-GPU:",
"                       #   GT pool + occupancy precomputed once, every optimizer step samples on-device).",
...
"STEPS_WARM  = 300      # warm-fit optimizer steps before pruning (runs fully on-GPU via the GT pool).",
"STEPS_REFIT = 80       # refit steps after each grow/prune (survivors close the gap).",
"MAX_PRUNE   = 14       # cap on greedy prune rounds so a stubborn mesh can't drain the per-mesh budget.",
"TIME_BUDGET = 45       # hard per-mesh wall-clock cap (s) so one hard mesh can't stall the whole run.",
...
"        max_prune=MAX_PRUNE, time_budget_s=TIME_BUDGET, min_keep=MIN_KEEP, device='cuda')",
